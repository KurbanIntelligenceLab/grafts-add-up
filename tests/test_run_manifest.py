...
class RunManifestTests(unittest.TestCase):
    ...
    def test_e2_requires_timestamps_and_retrospective_marks_unknown(self) -> None:
        from suture.run_manifest import write_retrospective_audit
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            run_dir.mkdir()
            (data_dir / "P_util.jsonl").write_text("{}\n", encoding="utf-8")
            (run_dir / "e2_report.json").write_text("{}", encoding="utf-8")
            ...
            output = write_retrospective_audit(
                ...
                language="es",
                command="python -m suture.tier_a_e2",
                reason="E2 completed before stage-specific manifests existed.",
            )
            payload = json.loads(output.read_text(encoding="utf-8"))
            self.assertIsNone(payload["started_at"])
            self.assertEqual(payload["timestamp_status"], "unknown")
