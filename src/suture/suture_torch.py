...
class HFResidualAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        shared_peft = getattr(self.host_model, "peft_config", None) is not None
        if self.host_model is self.donor_model and shared_peft and (
    ...
    def donor_block(self, l: int, x: np.ndarray) -> np.ndarray:
        ...
        if self.donor_model is self.host_model and self.donor_adapter_name == self.host_adapter_name:
            # Re-running an identical BF16 decoder block can accumulate
            # nondeterministic GPU roundoff.  The identical route is exactly
            # the host route, so reuse its captured update and make the
            # zero-donor invariant explicit.
            return self.host_block(l, x)
    ...
    def score_selection(
        ...
        try:
            from suture.suture_metrics import suture_scores
...
class RoutedGraftModel:
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        """Generate through the routed host/donor trajectory.

        The loop is intentionally explicit instead of delegating to
        ``PreTrainedModel.generate``.  The latter installs and removes cache
        hooks around multiple internal forwards, which is unsafe for a
        zero-copy layer-routing context on some PEFT/Windows combinations.
        Cached and uncached greedy tokens are required to match; per-step
        logits on grafted layers may differ at a documented numerical
        tolerance because the donor replacement hook still lets the host
        layer run first.
        """

        max_new_tokens = int(kwargs.pop("max_new_tokens", 1))
        ...
        return_step_logits = bool(kwargs.pop("return_step_logits", False))
        eos_token_id = kwargs.pop(
            "eos_token_id",
            getattr(self.adapter.host_model.config, "eos_token_id", None),
        )
        kwargs.pop("pad_token_id", None)
        if kwargs:
            raise AdapterError(
                f"unsupported routed generation arguments: {sorted(kwargs)}"
            )
        if max_new_tokens < 0:
            raise AdapterError("max_new_tokens must be non-negative")
        if do_sample and temperature <= 0:
            raise AdapterError("temperature must be positive when sampling")
        if not 0 < top_p <= 1:
            raise AdapterError("top_p must be in the interval (0, 1]")
        generated = input_ids.to(self.adapter.device)
        if generated.ndim == 1:
            generated = generated.unsqueeze(0)
        if attention_mask is None:
            mask = torch.ones_like(generated)
        else:
            mask = attention_mask.to(self.adapter.device)
            if mask.ndim == 1:
                mask = mask.unsqueeze(0)
        ...
        with torch.inference_mode():
            for _ in range(max_new_tokens):
                ...
                if past_key_values is not None:
                    position_ids = position_ids[:, -step_input_ids.shape[1] :]
                ...
                logits = getattr(outputs, "logits", None)
                if not isinstance(logits, Tensor):
                    raise AdapterError("routed generation did not return .logits")
                ...
                if eos_token_id is not None:
                    ...
                    if torch.all(torch.isin(next_token[:, 0], eos)):
                        break
        if return_step_logits:
            if not step_logits:
                empty = generated.new_zeros(
                    (generated.shape[0], 0, self.adapter.host_model.config.vocab_size)
                )
                return generated, empty
            return generated, torch.stack(step_logits, dim=1)
        return generated
    ...
    def sequence_logprob(
        ...
        """Return teacher-forced log-probability for every target sequence."""
        ...
        if prompt_input_ids.shape[0] != target_input_ids.shape[0]:
            raise AdapterError("prompt and target batch sizes must match")
        if target_input_ids.shape[1] == 0:
            raise AdapterError("target sequence must contain at least one token")
        if prompt_attention_mask is None:
            prompt_attention_mask = torch.ones_like(prompt_input_ids)
        elif prompt_attention_mask.ndim == 1:
            prompt_attention_mask = prompt_attention_mask.unsqueeze(0)
        full_input_ids = torch.cat(
            [prompt_input_ids, target_input_ids],
            dim=1,
        ).to(self.adapter.device)
        full_attention_mask = torch.cat(
            [
                prompt_attention_mask.to(self.adapter.device),
                torch.ones(
        ...
        with torch.no_grad():
            ...
            logits = getattr(outputs, "logits", None)
            if not isinstance(logits, Tensor):
                raise AdapterError("routed sequence score did not return .logits")
            start = prompt_input_ids.shape[1] - 1
            target_logits = logits[:, start : start + target_input_ids.shape[1], :]
            log_probs = torch.log_softmax(target_logits.float(), dim=-1)
            selected = log_probs.gather(
                2,
                target_input_ids.to(self.adapter.device).unsqueeze(-1),
            ).squeeze(-1)
